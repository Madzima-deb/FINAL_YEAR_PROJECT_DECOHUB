<script lang="ts">
    ...
    let objProps = $state({ x: 0, y: 0, z: 0, rx: 0, ry: 0, rz: 0, scale: 1, width: 0.64 });
    ...
    async function loadDesign(design: SavedDesign) {
        ...
        for (const item of design.items) {
            if (!item.isDuplicate) {
                ...
                sceneManager.applyTransform(item.sourceModelId, item.position, item.rotation, item.scale);
                ...
                sceneManager.applyTransform(newId, item.position, item.rotation, item.scale);
    ...
    onMount(() => {
        ...
        sceneManager.onObjectSelected = (id) => {
            ...
            if (id) {
                ...
                const t = transforms.get(id);
                if (t) {
                    const scaleMult = sceneManager.getModelScaleMultiplier(id);
                    const phys = sceneManager.getModelPhysicalSize(id);
                    objProps = {
                        ...
                        scale: Number(scaleMult.toFixed(2)),
                        width: phys.width
                    };
                }
            }
    ...
    function updateObjProps() {
        ...
        // Scale is applied separately through applyXZScale so height stays locked.
        sceneManager.applyTransform(
            ...
            [objProps.scale, objProps.scale, objProps.scale]
        );
        // Apply proportional X/Z scale multiplier
        sceneManager.applyXZScale(activeObjectId, objProps.scale);
        const phys = sceneManager.getModelPhysicalSize(activeObjectId);
        objProps.width = phys.width;
    }

    function updateObjWidth() {
        if (!activeObjectId || !sceneManager || objProps.width <= 0) return;
        sceneManager.setModelPhysicalWidth(activeObjectId, objProps.width);
        const scaleMult = sceneManager.getModelScaleMultiplier(activeObjectId);
        objProps.scale = Number(scaleMult.toFixed(2));
    }
        ...
        {#if activeObjectId}
            <div class="category properties-panel">
                ...
                <div class="prop-group">
                    ...
                    <div class="prop-row">
                        <input id="prop-scale" type="range" min="0.1" max="5" step="0.05" bind:value={objProps.scale} oninput={updateObjProps}>
                        <span class="scale-val">{objProps.scale.toFixed(2)}</span>
                    </div>
                </div>
                <div class="prop-group">
                    <label for="prop-width">Width (m)</label>
                    <div class="prop-row">
                        <input id="prop-width" type="number" min="0.1" max="10" step="0.02" bind:value={objProps.width} oninput={updateObjWidth}>
                        <span class="dim-val">{(objProps.width || 0).toFixed(2)} m</span>
                    </div>
                </div>
            </div>
        {/if}
...
<style>
    ...
    .scale-val { font-size: 0.85rem; color: #333; width: 36px; text-align: right; }
    .dim-val { font-size: 0.85rem; color: #555; white-space: nowrap; min-width: 48px; text-align: right; }
